...
│   │   └── game_archive.py        # 归档层: 对局历史归档（间接：进化管线消费→产出策略→注入prompt）
│   ├── core/                      # 核心数据结构
│   │   ├── enums.py               # 角色/阶段/事件枚举
...
从临时到持久，直接注入 Prompt 的记忆分为三层，另有归档层间接服务进化管线：
...
├─────────────────────────────────────────────────────┤
│ 归档层: 对局归档 (Game Archive) — 不直接注入 Prompt     │
│ 范围: 永久  |  存储: MySQL  |  延迟: ~50ms             │
│ 内容: 完整对局 trace，含策略使用和版本追踪              │
│ 用途: 进化管线消费归档数据→产出进化策略→注入 Prompt     │
│ 价值: 间接驱动 Agent 进化，是自进化闭环的数据基石       │
└─────────────────────────────────────────────────────┘
```
...
**解法**：三层记忆直接注入 Prompt（瞬时→跨局逐层沉淀），归档层间接服务进化管线（消费归档→产出策略→注入），策略按需加载（索引→全文→版本对比），信息密度最大化。
